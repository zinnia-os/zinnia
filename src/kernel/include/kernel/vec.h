...
// Append an element. Returns ZN_OK or ZN_ERR_NO_MEMORY.
#define VEC_PUSH(vec, val) \
    ({ \
        zn_status_t _status = ZN_OK; \
        if ((vec)->length >= (vec)->capacity) { \
            _VEC_GROW(vec, (vec)->length + 1); \
            if ((vec)->data == nullptr) \
                _status = ZN_ERR_NO_MEMORY; \
        } \
        if (_status == ZN_OK) \
...
// Insert at index, shifting subsequent elements right. Returns ZN_OK or ZN_ERR_NO_MEMORY.
